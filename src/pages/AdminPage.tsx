...
import { FerienwohnungenViewDialog } from '@/components/dialogs/FerienwohnungenViewDialog';
...
const FERIENWOHNUNGEN_FIELDS = [
  { key: 'bezeichnung', label: 'Bezeichnung', type: 'string/text' },
  { key: 'beschreibung', label: 'Beschreibung', type: 'string/textarea' },
  { key: 'max_personen', label: 'Maximale Personenanzahl', type: 'number' },
  { key: 'lage', label: 'Lage / Adresse', type: 'string/text' },
  { key: 'hinweise', label: 'Besondere Hinweise', type: 'string/textarea' },
];
const BUCHUNGEN_FIELDS = [
  { key: 'ferienwohnung', label: 'Ferienwohnung', type: 'applookup/select', targetEntity: 'ferienwohnungen', targetAppId: 'FERIENWOHNUNGEN', displayField: 'bezeichnung' },
...
const ENTITY_TABS = [
  { key: 'ferienwohnungen', label: 'Ferienwohnungen', pascal: 'Ferienwohnungen' },
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('ferienwohnungen');
  const [selectedIds, setSelectedIds] = useState<Record<EntityKey, Set<string>>>(() => ({
    'ferienwohnungen': new Set(),
    'buchungen': new Set(),
  }));
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'ferienwohnungen': return (data as any).ferienwohnungen as Ferienwohnungen[] ?? [];
  ...
  const getLookupLists = useCallback((entity: EntityKey) => {
    ...
    switch (entity) {
      case 'buchungen':
        lists.ferienwohnungenList = (data as any).ferienwohnungen ?? [];
        break;
    }
    return lists;
  }, [data]);
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, fieldKey: string, url?: unknown) => {
    ...
    if (entity === 'buchungen' && fieldKey === 'ferienwohnung') {
      const match = (lists.ferienwohnungenList ?? []).find((r: any) => r.record_id === id);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'ferienwohnungen': return FERIENWOHNUNGEN_FIELDS;
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'ferienwohnungen': return {
        create: (fields: any) => LivingAppsService.createFerienwohnungenEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateFerienwohnungenEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteFerienwohnungenEntry(id),
      };
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'ferienwohnungen' || dialogState?.entity === 'ferienwohnungen') && (
        <FerienwohnungenDialog
          open={createEntity === 'ferienwohnungen' || dialogState?.entity === 'ferienwohnungen'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'ferienwohnungen' ? handleUpdate : (fields: any) => handleCreate('ferienwohnungen', fields)}
          defaultValues={dialogState?.entity === 'ferienwohnungen' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['Ferienwohnungen']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Ferienwohnungen']}
        />
      )}
      {(createEntity === 'buchungen' || dialogState?.entity === 'buchungen') && (
        <BuchungenDialog
          ...
          ferienwohnungenList={(data as any).ferienwohnungen ?? []}
          enablePhotoScan={AI_PHOTO_SCAN['Buchungen']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Buchungen']}
        />
      )}
      {viewState?.entity === 'ferienwohnungen' && (
        <FerienwohnungenViewDialog
          open={viewState?.entity === 'ferienwohnungen'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'ferienwohnungen', record: r }); }}
        />
      )}
