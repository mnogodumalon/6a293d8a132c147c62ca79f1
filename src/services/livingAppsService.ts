...
import type { Ferienwohnungen, Buchungen, CreateFerienwohnungen, CreateBuchungen } from '@/types/app';
...
export class LivingAppsService {
  // --- FERIENWOHNUNGEN ---
  static async getFerienwohnungen(): Promise<Ferienwohnungen[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.FERIENWOHNUNGEN}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Ferienwohnungen[];
    return enrichLookupFields(records, 'ferienwohnungen');
  }
  static async getFerienwohnungenEntry(id: string): Promise<Ferienwohnungen | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.FERIENWOHNUNGEN}/records/${id}`);
    const record = { record_id: data.id, ...data } as Ferienwohnungen;
    return enrichLookupFields([record], 'ferienwohnungen')[0];
  }
  static async createFerienwohnungenEntry(fields: CreateFerienwohnungen) {
    return callApi('POST', `/apps/${APP_IDS.FERIENWOHNUNGEN}/records`, { fields: cleanFieldsForApi(fields as any, 'ferienwohnungen') });
  }
  static async updateFerienwohnungenEntry(id: string, fields: Partial<CreateFerienwohnungen>) {
    return callApi('PATCH', `/apps/${APP_IDS.FERIENWOHNUNGEN}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'ferienwohnungen') });
  }
  static async deleteFerienwohnungenEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.FERIENWOHNUNGEN}/records/${id}`);
  }
